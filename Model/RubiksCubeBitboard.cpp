...
// Implements a Rubik's Cube using a 64-bit bitboard representation per face.
// Each face stores 8 stickers (the center is fixed and implied by the face index).
...
private:
    ...
    // Mapping from (row, col) to sticker index in the bitboard encoding.
    // The order is clockwise: top-left → top-right → right → bottom-right → bottom → bottom-left → left.
    int arr[3][3] = {{0, 1, 2},
                     {7, 8, 3},     // center (8) is unused
                     {6, 5, 4}};
    ...
    // Only rotates the 8 stickers around the center.
    void rotateFace(int ind) {
        uint64_t side = bitboard[ind];
        side = side >> (8 * 6);     // move last 2 bytes to lowest position
        bitboard[ind] = (bitboard[ind] << 16) | (side);
    }

    // Rotate stickers along the edges between two faces.
    void rotateSide(int s1, int s1_1, int s1_2, int s1_3, int s2, int s2_1, int s2_2, int s2_3) {
        // Extract three 8-bit color values from s2
        ...
        // Place them into the positions of s1
    ...
    // Encodes a corner piece using 5 bits based on its three colors.
    // Each bit encodes a color identity and orientation.
    int get5bitCorner(string corner) {
        int ret = 0;
        string actual_str;

        // Encode white/yellow info (bit 2)
        for (auto c: corner) {
            if (c != 'W' && c != 'Y') continue;
...
public:
    // Bitboard representation of the cube (one 64-bit value per face).
    uint64_t bitboard[6]{};
    ...
    // Returns the color of a sticker at a given (face, row, col)
    COLOR getColor(FACE face, unsigned row, unsigned col) const override {
        int idx = arr[row][col];
        if (idx == 8) return (COLOR)((int) face);       // center sticker is always face color
    ...
    // Encodes the 8 corner pieces using 5 bits each (total 40 bits).
    uint64_t getCorners() {
        uint64_t ret = 0;
