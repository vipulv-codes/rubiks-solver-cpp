...
private:
    ...
    void rotateFace(int ind) {
        ...
        // Copy the face into a temporary array
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
...
public:
    // 3D array to store colors of each face: 6 faces, 3x3 each
    char cube[6][3][3]{};

    // Initializes the cube to 'already' solved state
    RubiksCube3dArray() {
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 3; j++) {
                for (int k = 0; k < 3; k++)
...
// Generates a unique hash based on the cube's current state.
/*
    Converts the cube's 3D array into a flat string representation
    by concatenating the color letters from each face, and hashes
