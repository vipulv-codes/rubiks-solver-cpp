...
// Represents the Rubik's Cube using a 1D array (size 54) (faster than 3D representation)
...
public:
    // 1D array of size 54 representing the cube
    char cube[54]{};

    // Initializes cube to 'already' solved state
    RubiksCube1dArray() {
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 3; j++) {
                for (int k = 0; k < 3; k++) {
...
// Hash function for 1D array representation
// Hashes the cube state by converting stickers to a string.
struct Hash1d {
